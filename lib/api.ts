...
export async function getTafsir(verseKey: string, language = "en"): Promise<string> {
  try {
    ...
    if (!response.ok) throw new Error(`Failed to fetch tafsir: ${response.status} ${response.statusText}`)
    const data = await response.json()
    ...
    return "Unable to load tafsir at this time. Please try again later."
  }
}
