  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 pb-16 md:pb-0">
        ...
        <section className="relative">
          ...
          <div className="container relative py-12 sm:py-20 md:py-28">
            <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
              ...
              <div className="relative mx-auto aspect-square max-w-md overflow-hidden rounded-full border-8 border-emerald-50 dark:border-emerald-950 lg:h-[450px] lg:w-[450px]">
                <Image
        ...
        <section className="py-8 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/30">
          <div className="container">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-8">
              <div className="bg-white dark:bg-background rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900 p-5 flex flex-col items-center justify-center text-center transition-transform hover:scale-105">
                <BookOpen className="h-8 w-8 text-emerald-600 mb-3" />
                <h3 className="text-2xl font-bold">{stats.totalSurahs}</h3>
                <p className="text-sm text-muted-foreground">Surahs</p>
              </div>
              <div className="bg-white dark:bg-background rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900 p-5 flex flex-col items-center justify-center text-center transition-transform hover:scale-105">
                <BookMarked className="h-8 w-8 text-emerald-600 mb-3" />
                <h3 className="text-2xl font-bold">{stats.totalVerses}</h3>
                <p className="text-sm text-muted-foreground">Verses</p>
              </div>
              <div className="bg-white dark:bg-background rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900 p-5 flex flex-col items-center justify-center text-center transition-transform hover:scale-105">
                <TrendingUp className="h-8 w-8 text-emerald-600 mb-3" />
                <h3 className="text-2xl font-bold">{stats.totalWords.toLocaleString()}</h3>
                <p className="text-sm text-muted-foreground">Words</p>
              </div>
              <div className="bg-white dark:bg-background rounded-xl shadow-sm border border-emerald-100 dark:border-emerald-900 p-5 flex flex-col items-center justify-center text-center transition-transform hover:scale-105">
                <Star className="h-8 w-8 text-emerald-600 mb-3" />
                <h3 className="text-2xl font-bold">{stats.totalLetters.toLocaleString()}</h3>
            ...
            <div className="md:col-span-8 space-y-10">
              ...
              <div className="space-y-6">
                ...
                {loading ? (
                  ...
                  <div className="grid gap-6 sm:grid-cols-3">
                    {featuredSurahs.slice(0, 3).map((surah) => (
                      <Link
                        key={surah.id}
                        href={`/read/${surah.id}`}
              ...
              <Card className="shadow-md">
                ...
                <CardContent className="pt-4 px-4">
                  <div className="space-y-3">
                    <Link
                      href="/bookmarks"
                      className="flex items-center justify-between rounded-lg border p-3.5 transition-colors hover:bg-muted"
                    >
              ...
              {loadingVerses
                ...
                : inspirationalVerses.map((verse, index) => (
                    <Card key={index} className="overflow-hidden shadow-md hover:shadow-lg transition-shadow">
                      <CardHeader className="pb-0 border-b bg-gradient-to-r from-emerald-50/50 to-teal-50/50 dark:from-emerald-950/20 dark:to-teal-950/20">
                        <CardTitle className="text-sm font-medium text-emerald-600 dark:text-emerald-400">
                          {verse.verse_key}
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="pt-5 px-5">
                        <p className="mb-5 text-right font-arabic text-xl leading-relaxed" dir="rtl">
                          {verse.text_uthmani}
                        </p>
